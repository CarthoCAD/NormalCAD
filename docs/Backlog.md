...
## Decouple `PropertyPalette` — DataTemplates + extract data/transaction logic (priority: high)

`PropertyPalette` is currently a "god control" that mixes presentation with data access, transaction control, input parsing, write-back, error reporting, and cross-control coordination — reaching across all three layers (View → Controller → Core). It builds its entire UI programmatically in ~330 lines of C# (`TextBox`/`ComboBox`/`Border`/`Grid` created with `new` in code-behind), bypassing Avalonia's styling system and compiled bindings; it opens Core transactions to materialize the selected entities (`OnSelectionChanged`) and to write values (`ApplyAndCommit`); it pushes messages into the command line via `InputManager.SetPromptMessage`; it calls `Viewport.InvalidateVisual` directly; and it resolves theme brushes by hand (`EnsureBrushes`). Rework it into a pure presentation layer:

- **UI (DataTemplates):** replace the imperative build (`BuildPropertyGrid`, `AddCategoryHeader`, `CreateTextEditor`, `CreateComboEditor`) with an `ItemsControl` bound to an `ObservableCollection` of lightweight, bindable row records (category, display name, editor kind, current value, combo options, and a commit handle), rendered by XAML `DataTemplate`s with `x:DataType` for compiled bindings. Category headers come from the templates. Note this is a bindable row model, **not** a full `PropertyPaletteViewModel` — a dedicated view-model is unnecessary in the current MVC/code-behind style.
- **Theming:** remove `EnsureBrushes`; resolve all brushes through `DynamicResource` in the XAML templates (also fixes the cached-brush staleness on theme change and the unresolved `_accentBrush`).
- **Data & transactions:** move entity materialization (read) and value write-back/commit into `EntityPropertyManager`, making it the single owner of the `Transaction` lifecycle. The palette calls something like `GetRowsForSelection(...)` and `TrySetProperty(descriptor, value)` and never touches `Database`/`TransactionManager`/`OpenMode`.
- **Parsing:** give the descriptor (or the manager's set method) responsibility for string → typed-value conversion, so the view stops switching on `PropertyType` (`OnTextEditorKeyDown`).
- **Signaling via events:** replace the direct `InputManager.SetPromptMessage` call on invalid input with an event or a returned result that the `CadController` routes to the prompt; drop the `Viewport.InvalidateVisual` call and rely on the existing `Database.Changed → CadController.OnDatabaseChanged → Viewport` redraw path.
- **Cleanup:** remove the dead `_selectedIds` field (written, never read).

The result is a palette with zero knowledge of entities, providers, the database, transactions, the viewport, or the command prompt — reusable for non-entity property sources (layers, blocks, external references) with no UI code changes. Depends on the Entity Property Manager as the source of the row/descriptor data.
